...
namespace Day11
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Func<long> getColor = () => colors.ContainsKey(currentPosition) ? colors[currentPosition] : 0;
            ...
            {
                result = reader.RunIntCode(getColor);
                colors[currentPosition] = result;

                // place to move
                result = reader.RunIntCode(getColor);
                Turn turn = (Turn) result;
                switch ((currentDirection, turn))
                {
