...
namespace Day15
{
    class Program
    {
        public static IntCodeReader Reader;

        public static Dictionary<(int, int), int> MinMovesToPosition;

        public static int DroidX;

        public static int DroidY;

        public static int CheeseX;

        public static int CheeseY;
        ...
        static void Main(string[] args)
        {
            ...
            Reader = new IntCodeReader(line);

            MinMovesToPosition = new Dictionary<(int, int), int>();
            DroidX = 0;
            DroidY = 0;
            ...
            for (int i = 1; i <= 4; i++)
            {
                ...
                switch (response)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        Console.WriteLine($"Found cheese at {DroidX}, {DroidY}.");
                        CheeseX = DroidX;
                        CheeseY = DroidY;
        ...
        private static bool SaveNewPosition(int oldX, int oldY) // returns if it's better than before and we should continue
        {
            if (!MinMovesToPosition.ContainsKey((DroidX, DroidY)))
            {
                MinMovesToPosition[(DroidX, DroidY)] = MinMovesToPosition[(oldX, oldY)] + 1;
                return true;
            }

            // strict inequality here, if they are equal we don't want to waste time.
            if (MinMovesToPosition[(DroidX, DroidY)] > MinMovesToPosition[(oldX, oldY)] + 1)
            {
                MinMovesToPosition[(DroidX, DroidY)] = MinMovesToPosition[(oldX, oldY)] + 1;
                return true;
            }
        ...
        private static void Advance(int direction)
        {
            switch (direction)
            {
                case 1:
        ...
        private static void Retreat(int direction)
        {
